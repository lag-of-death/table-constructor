...
const getActions = (actionHandler, idx) => [[BACK_ACTION, '+'], [REMOVE_ACTION, '-'], [FRONT_ACTION, '+']].map(action => (
  <Action
    key={`${action}:${idx}`}
...
const TableContent = ({
  rows, handleColumnValueChange, handleColumnAction, handleRowAction,
}) => (
  rows.map((row, rowIdx) => (
            ...
            >
              {
                getRowContent(
                  ...
                  getColumnActions.bind(null, handleColumnAction),
                )
              }
            </RowHeader>
          )
